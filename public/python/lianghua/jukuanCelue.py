...
def sell_heavy_turnover(context):
    ...
    for s in stock_list:
        df = get_price(s, end_date=date, fields=['high', 'low', 'close', 'open', 'volume', 'high_limit'], count=2,
                       fill_paused=False, skip_paused=False, panel=False)
        ...
        if ((context.portfolio.positions[s].closeable_amount != 0) and (vol_ratio >= 0.5) and (close_pos <= 0.65) and (
                current_data[s].last_price < df['close'][-1])):
            order_target_value(s, 0)
